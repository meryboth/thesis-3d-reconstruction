...
Con el fin de profundizar acerca de la propuesta de segmentación para la nube de puntos densa de SfM es importante entender el estado del arte que nos llevó a plantear la integración con HBIM y el proceso de BIM como un flujo de trabajo posible de mejorar: recorrer la nube completa e identificar a mano qué región corresponde a cada elemento constructivo (cubierta, columna, muro, piso) antes de empezar a levantar geometría paramétrica es un proceso tedioso y artesanal en su ejecución. Este cuello de botella está documentado en la literatura de *scan-to-BIM* para patrimonio —[Romero-Jarén y Arranz (2021)](#ref-romero-jaren-2021) lo describen como el paso "muy costoso en tiempo y enteramente delegado al trabajo manual de expertos, lejos de estar automatizado". Este descubrimiento motivó una prueba de concepto adicional dentro de esta tesis: un clasificador geométrico que segmenta automáticamente la nube densa de SfM en las cuatro clases relevantes para BIM antes de exportarla como referencia.
...
Lo que se puede identificar a partir de la Figura 6.5 demuestra que, si el edificio tiene un contexto cercano de vegetación, estos puntos son interpretados dentro de la segmentación como partes de la arquitectura. Queda en evidencia la necesidad de incluir un modo de edición manual para poder continuar optimizando la nube de puntos, con ese fin se incluye en el visor la posibilidad de eliminar puntos y guardar el modelo (Figura 6.6). Esta solución está fundada por la literatura: [Croce et al. (2023)](#ref-croce-2023) y [Pan et al. (2024)](#ref-pan-2024) framean sus propuestas como *"semi-automáticas"* precisamente porque ningún clasificador —ni el geométrico simple de esta tesis, ni los basados en aprendizaje profundo— elimina la necesidad de una revisión humana antes de que el resultado se use como base de un modelo BIM.
...
Otro de los experimentos que se llevaron adelante para esta tesis tiene como protagonista a la posibilidad de automatizar el proceso de modelado 3D con agentes conectados al MCP de programas de modelo, como es el caso de Blender. La prueba de concepto que se realizó se hizo utilizando el último modelo lanzado hasta la fecha por OpenAI llamado GPT-6 Astra (desde Codex), y su conexión mediante MCP con Blender. El objetivo de este experimento es automatizar el flujo de modelado 3D pero utilizando como base dos componentes claves de este pipeline: la nube de puntos para determinar la precisión del modelo y que los agentes puedan atender la geometría, y el dataset original utilizado para los entrenamientos con el fin que el LLM pueda consultar detalles arquitectónicos y validar visualmente como se ve la obra construida. 
...
![Nube de puntos densa de Templete Central importada y orientada en Blender como referencia](media/templete-proceso-01-nube-alineada.png)
...
![Geometría principal de columnas y cubierta ajustada sobre la nube, antes de agregar relieves y accesos](media/templete-proceso-02-estructura-ajustada.png)

*Figura 6.12 — Templete Central, paso 2: geometría principal ajustada, antes de los relieves y accesos.*

![Superposición de la nube de puntos del edificio con las aristas del modelo, usada para verificar la alineación](media/templete-proceso-03-superposicion.png)

*Figura 6.13 — Templete Central, paso 3: superposición de la nube y las aristas del modelo para comprobar alineación.*

![Modelo con el patrón de relieves de la cubierta y los accesos ya agregados como detalle interpretativo](media/templete-proceso-04-relieves-accesos.png)

*Figura 6.14 — Templete Central, paso 4: etapa de detalle interpretativo (relieves y accesos).*

![Escena final del modelo de Templete Central en Blender tras la revisión geométrica y visual](media/templete-proceso-05-final.png)
...
![Nube de puntos densa del Panteón Asociación Catalana importada y orientada en Blender como referencia](media/panteon-proceso-01-nube-alineada.png)

*Figura 6.16 — Panteón Asociación Catalana, paso 1: referencia fotogramétrica importada y orientada.*

![Volúmenes arquitectónicos principales ajustados sobre la nube del Panteón](media/panteon-proceso-02-volumenes.png)

*Figura 6.17 — Panteón Asociación Catalana, paso 2: volúmenes principales ajustados.*

![Superposición de la nube de puntos del Panteón con las aristas del modelo](media/panteon-proceso-03-superposicion.png)
...
![Ornamentos del Panteón refinados en Blender: follaje, volutas, medallón e inscripción sobre la fachada](media/panteon-proceso-04-ornamentos.png)
...
![Render final del modelo reconstruido del Panteón Asociación Catalana, con fachada, cúpula, linterna, escalinata y anexo circular](media/panteon-proceso-05-final.png)
