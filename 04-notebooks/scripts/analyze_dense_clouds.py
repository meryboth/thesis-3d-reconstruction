...
def make_scatter_plot(path, sample, case):
    ...
    axes[0].set_xlabel("X")
    axes[0].set_ylabel("Y")
    axes[0].set_title(f"{case} — proyeccion XY (muestra)")
    axes[0].set_aspect("equal", adjustable="datalim")

    axes[1].scatter(sample[:, 0], sample[:, 2], s=0.15, alpha=0.4, color="#2c3e50")
    axes[1].set_xlabel("X")
    axes[1].set_ylabel("Z")
    axes[1].set_title(f"{case} — proyeccion XZ (muestra)")
    axes[1].set_aspect("equal", adjustable="datalim")

    fig.tight_layout()
    out = path.parent / f"{path.stem}-scatter.png"
...
def analyze_cloud(case, cfg):
    ...
    plot_path = make_scatter_plot(path, base["sample"], case)

    print(f"[OK] Points: {base['point_count']:,}")
    print(f"[OK] Log:  {log_path}")
    print(f"[OK] JSON: {json_path}")
    print(f"[OK] PNG:  {plot_path}")
