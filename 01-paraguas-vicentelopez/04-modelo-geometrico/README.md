...
- Generador: `../../04-notebooks/scripts/reconstruir_paraguas_blender.py`. Se ejecuta dentro de Blender, con un diccionario compartido de globals/locals para `exec`. Lee los parámetros JSON y crea una escena nueva. Las referencias se incorporan si ya están importadas con los nombres usados en esta sesión.

## Referencias y procedimiento

Fuente geométrica: `../02-resultados-finales/colmap-fotogrametria-densa/fused_medium_high_clean.ply` (502.817 puntos). La malla `meshed-poisson-clean-trim5.ply` sirve para comparación visual; sus artefactos no se copian al modelo.
